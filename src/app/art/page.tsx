...
export default function ManageArt() {
  ...
  const filters = [ 'Title', 'Location', 'Medium', 'Aquired', 'Created', 'Height', 'Width', 'Depth' ]
  ...
  return (
    <main className='dark:text-white mt-2'>
      ...
      <div className="grid-cols-2 max-w-[1150px] my-2 sm:ml-[170px] p-2 max-sm:mx-4 sm:w-2/3 max-sm:max-h-52 md:grid-cols-3 xl:grid-cols-5 gap-x-2 max-sm:overflow-y-scroll bg-slate-100 dark:bg-slate-800 border border-slate-400 dark:border-slate-600 hidden rounded-lg" id="filters">
        <div>
      ...
      {modalArt && 
        <div id='modal' className='fixed w-screen h-screen top-0 right-0 bg-black/80'>
          <div tabIndex={-1} onBlur={ () => setModalArt(undefined) } className='absolute w-3/4 h-3/4 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-800 p-4 border border-black rounded-lg focus-visible:outline-none'>
            ...
            <div id='buttons' className="fixed right-3 bottom-3">
              ...
              <button className="btn-gray" onClick={ () => setModalArt(undefined) }>Close</button>
            </div>
          </div>
        </div>
      }
